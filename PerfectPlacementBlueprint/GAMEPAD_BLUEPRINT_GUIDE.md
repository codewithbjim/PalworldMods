...
captures these three physical chords:
...
- unfrozen Freeze, Copy, and Freeze into Piece may be remapped among those
  three captured chords;
...
Gamepad input is event-driven. Do not add `poll_interval_ms`, a permanent
`LoopAsync`, or any other recurring Lua serial monitor.
...
Keep the existing entries in this exact order. Their numeric values `0..21`
must not change. Append the new unfrozen chord as value `22`:
...
Keep the existing `KeyGuideOverlay` styling. `GamepadUnfrozenKeyGroups` has
three `KeyGuideOverlay` children. Each frozen grouping Vertical Box has exactly
two; Frozen Unfreeze is the only standalone row outside the three frozen
grouped columns.
...
└─ GamepadUnfrozenKeyGroups                         VerticalBox
   ...
   └─ GamepadCopyFreezeRow                          KeyGuideOverlay
      └─ RowContent
         └─ GamepadCopyFreezeContent                HorizontalBox
            ├─ GamepadCopyFreezeKeycaps             HorizontalBox
            │  └─ GP_UnfrozenCopyFreezeChord
            └─ GamepadCopyFreezeLabel               "Freeze into Piece"
...
## 8. Name the fifteen reusable instances
...
increment. Compile the parent widget and confirm all fifteen chord instances
...
GamepadUnfrozenCopyFreezeSerial
...
These are physical input counters. Prefer physical names rather than action
names such as `GamepadMoveUpSerial`, which would hard-code the mapping in
Blueprint. `GamepadUnfrozenCopyFreezeSerial` is the compatibility exception
already shipped in the current pak; the enum remains the authoritative physical
input.
...
| `Unfrozen_L3_DPadUp` | `GamepadUnfrozenCopyFreezeSerial` |

Compile after wiring all 23 branches. Keep `Unfrozen_L3_DPadUp` last so the
numeric values of the original 22 entries remain unchanged.

Lua may also accept `GamepadUnfrozenL3DPadUpSerial` as a compatibility alias
for intermediate Blueprint builds. The current pak's primary property is
`GamepadUnfrozenCopyFreezeSerial`; do not require both properties.
...
└─ true:
   ...
   → Set bL3CompoundChordUsed = true
```

Add D-pad Up Pressed:

```text
Branch(bL3Held)
├─ false → Return
└─ true:
   KeyGuideWidget.QueueGamepadPhysicalInput(Unfrozen_L3_DPadUp)
   → Set bL3CompoundChordUsed = true
```

The delayed L3 release distinguishes an L3 tap from either compound chord. It
prevents Copy or Freeze into Piece from also queuing plain Freeze.

Set `Consume Input = false` on all three unfrozen events. This lets the events
...
```text
/Game/Mods/PerfectPlacement/WBP_PerfectPlacement_KeyGuide.WBP_PerfectPlacement_KeyGuide_C:QueueGamepadPhysicalInput
```

For this Blueprint function, UE4SS invokes the callback supplied as argument 2
after the Blueprint function has run and ignores argument 3. Retain the callback
record and both hook IDs returned by `RegisterHook`; do not describe this as a
true pre-hook.

Decode the `EPP_GamepadPhysicalInput` argument directly when UE4SS exposes it
reliably. Otherwise, baseline all 23 widget serials when attaching a new host
and compare them only when this hook fires. Since the Blueprint function has
just incremented exactly one serial, this fallback is still event-driven and
does no idle work.

Lua resolves the physical input through:
...
- maps `Unfrozen_L3_DPadUp` to the configured `freeze_to_piece` action;
- dispatches through the retained EngineTick action path instead of mutating the
  preview inside the Blueprint hook;
- never replays serial values observed before the current widget was attached;
- refreshes action-row keycaps from the effective configuration.

Do not run a permanent serial poll. If hook registration fails, disable
gamepad actions and log the failure once rather than silently starting a timer.
...
4. The parent widget exposes all fifteen exact chord-instance names.
5. Unfrozen contains exactly the Freeze, Copy, and Freeze into Piece overlay
   rows.
...
7. L3+Up while unfrozen queues only `Unfrozen_L3_DPadUp`.
8. Releasing L3 after either unfrozen compound chord does not also queue plain
   L3.
...
6. Test unfrozen Freeze, Copy, and Freeze into Piece.
...
14. Temporarily log the Lua hook and confirm each physical press produces
    exactly one callback and one action, including `Unfrozen_L3_DPadUp`.
15. Spam frozen movement, rotation, reset, and unfreeze; confirm the retained
    hook and EngineTick action dispatcher remain responsive without duplicate
    actions or callback-GC errors.
16. Recreate the guide by leaving and re-entering build mode; confirm serials
    are baselined and no input from the previous widget is replayed.
17. Leave the controller idle for at least 30 seconds in unfrozen and frozen
    modes; confirm no recurring Lua gamepad work, log cadence, or serial reads.
18. Exit build mode and confirm both input actors are disabled.
